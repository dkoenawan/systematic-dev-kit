...
bash /path/to/compass-labs/skills/doc-maintainer/scripts/install-schedule.sh \
...
```bash
bash /path/to/compass-labs/skills/doc-maintainer/scripts/uninstall-schedule.sh \
...
/bin/bash /path/to/compass-labs/skills/doc-maintainer/scripts/maintain.sh /path/to/repo
...
| --- | --- | --- |
| `/compass:doc-maintainer` (no arg) | auto-detect | If `docs/explanation/solution-design.md` is absent on `chore/claude-maintain`: run init. Otherwise: run maintain. |
| `/compass:doc-maintainer init` | init | Whole-system exploration → write L1 `docs/explanation/solution-design.md` only. |
| `/compass:doc-maintainer maintain` | maintain | Priority-queue dispatch: generate next missing L2/L3, patch stale docs, or run clarity review — one file per run. |
| `/compass:doc-maintainer refresh` | refresh | Full re-exploration and rewrite of all docs. |
| `/compass:doc-maintainer refresh <domain>` | refresh-one | Full rewrite of one named domain's `docs/explanation/<domain>/overview.md` only. |
...
7. **If an explanation candidate (undocumented architectural decision) was found**, do not silently drop it — note it in the fold-back commit body: `possible undocumented decision: <one-line summary> — consider /compass:adr`. Do not invoke `/adr` automatically; that requires human judgment.
...
```html
<!-- Generated by compass-labs doc-maintainer. Do not edit manually — re-generated on each maintain run. Last updated: {today YYYY-MM-DD} -->
```
